...
# using resource to generate private key 
...
# create an instance depending on the subnet cidr
resource "aws_instance" "my_servers" {
  ...
  user_data = <<-EOF
              #!/bin/bash
              hostnamectl set-hostname ${var.project_name}_server${count.index+1}
              EOF
  ...
  #             sudo bash -c 'echo THIS IS WEB SERVER_${count.index+1} > /var/www/html/index.html'
  #             EOF
...
resource "aws_instance" "stand_alone" {
  ami                    = data.aws_ami.ubuntu.id
  instance_type          = var.instance_type
  key_name               = var.keypair_name
  vpc_security_group_ids = [aws_security_group.my_sg.id]
  subnet_id              = var.subnet_id[0]
  depends_on             = [aws_instance.my_servers]
  ...
  tags = {
    Name = "${var.project_name}_stand_alone"
  } 
}
